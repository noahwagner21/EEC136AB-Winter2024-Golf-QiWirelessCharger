...
int main(void)
{
  ...
  while (1)
  {
	...
	if (button ==0){ //if button pressed, write a 1 to the GPIO output pin
		HAL_GPIO_WritePin(GPIOC, GPIO_PIN_2, GPIO_PIN_SET);
		HAL_GPIO_WritePin(GPIOC, GPIO_PIN_3, GPIO_PIN_SET);
	}

	green = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_1);
	red = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_0);
	...
	if (delay_100 == 30){
		...
		if (num_changes >=2){
			Lcd_clear(&lcd);
			Lcd_cursor(&lcd, 0,0);
			Lcd_string(&lcd, "Charging in");
			Lcd_cursor(&lcd, 1, 0);
			Lcd_string(&lcd, "progress!");
			num_changes = 0;
		}
		else if((green==1)&&(red==0)&&(num_changes<2)){
			Lcd_clear(&lcd);
			Lcd_cursor(&lcd, 0,0);
			Lcd_string(&lcd, "Charging done!");
		}
		else if ((num_changes <2) && (red==0)&&(green==0)) {
			Lcd_clear(&lcd);
			Lcd_cursor(&lcd, 0,0);
			Lcd_string(&lcd, "Standby");
		}
		else if ((num_changes <2) && (red==1) && (green==0)){
			Lcd_clear(&lcd);
			Lcd_cursor(&lcd, 0,0);
